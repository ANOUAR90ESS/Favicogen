    ...
    <div className="flex flex-col h-full bg-white border-r border-slate-200 w-full overflow-hidden">
      ...
      <div className="grid grid-cols-4 border-b border-slate-200 bg-slate-50/80 p-1.5 gap-1 shrink-0">
        <button
          ...
          className={`flex flex-col sm:flex-row items-center justify-center gap-1.5 py-2 px-1 rounded-lg text-xs font-bold transition-all ${
            activeTab === 'icon'
        ...
        >
          ...
          <span className="text-[11px] truncate">{isAr ? 'العنصر/الأيقونة' : 'Icon/Image'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[11px] truncate">{isAr ? 'النصوص' : 'Text'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[11px] truncate">{isAr ? 'الخلفية والألوان' : 'Background'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[11px] truncate">{isAr ? 'التخطيط' : 'Layout'}</span>
        </button>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                ...
                <span>{isAr ? 'عنصر الشعار الرسومي' : 'Logo Graphic Element'}</span>
              </h3>
              ...
              <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-4 text-center space-y-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-700">
                    {isAr ? 'ارفع ملف SVG مخصص' : 'Upload custom SVG'}
                  </p>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    {isAr ? 'اسحب الملف هنا أو اضغط للاختيار' : 'Drag & drop file or browse'}
                  </p>
                </div>
                <input
                  type="file"
                ...
                <button
                  ...
                  className="rounded-lg bg-indigo-50 border border-indigo-200 px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 transition-colors"
                >
                  {isAr ? 'اختيار ملف SVG' : 'Choose SVG File'}
                </button>
              </div>
            )}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {['🚀', '⚡', '👑', '💎', '🔥', '✨', '🧠', '🌐', '🌿', '🎯', '🦁', '🛡️'].map((em) => (
                      <button
                        ...
                        className="text-lg p-1.5 rounded-lg bg-slate-50 border border-slate-200 hover:bg-white hover:border-slate-300 hover:scale-105 transition-all shadow-2xs"
                      >
                        {em}
                      </button>
                    ))}
                ...
                <div className="space-y-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={isAr ? 'ابحث عن أيقونة (معالج, تاج, درع...)' : 'Search icons (cpu, crown, shield...)'}
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="text-slate-700 font-bold">{isAr ? 'حجم العنصر' : 'Size'}</span>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    {isAr ? 'ألوان الأيقونة' : 'Icon Color & Gradient'}
                  </span>
                  <label className="flex items-center gap-1.5 text-xs text-slate-600 font-semibold cursor-pointer">
                    ...
                    />
                    <span>{isAr ? 'تدرج لوني' : 'Gradient'}</span>
                  </label>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <AdvancedColorPicker
                    label={isAr ? 'اللون الرئيسي' : 'Primary Color'}
                  ...
                  {config.iconGradient && (
                    <AdvancedColorPicker
                      label={isAr ? 'لون التدرج 2' : 'Gradient Color 2'}
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                {isAr ? 'المؤثرات البصرية (Shadows & Outlines)' : 'Shadows & Outlines'}
              </span>

              {/* Shadow Controls */}
              <div className="p-3 bg-slate-50/70 border border-slate-200 rounded-xl space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-700">{isAr ? 'تفعيل الظل (Drop Shadow)' : 'Drop Shadow'}</span>
                  <input
                    type="checkbox"
                ...
                {config.iconShadow && (
                  <div className="space-y-2">
                    ...
                    <AdvancedColorPicker
                      label={isAr ? 'لون الظل' : 'Shadow Color'}
              ...
              <div className="p-3 bg-slate-50/70 border border-slate-200 rounded-xl space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-700">{isAr ? 'إطار خارجي (Outline)' : 'Outline Stroke'}</span>
                  <input
                    type="checkbox"
                ...
                {config.iconOutline && (
                  <div className="space-y-2">
                    <div className="space-y-1">
                      <div className="flex justify-between text-[11px] text-slate-500 font-medium">
                        <span>{isAr ? 'سُمك الإطار' : 'Outline Width'}</span>
                    ...
                    <AdvancedColorPicker
                      label={isAr ? 'لون الإطار' : 'Outline Color'}
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100">
              <div className="flex items-center justify-between text-xs font-bold text-slate-700">
                <span>{isAr ? 'الموضع والتحويل' : 'Transform & Position'}</span>
                <button
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-[11px] text-slate-500 font-medium">
                  <span>{isAr ? 'دوران الزاوية' : 'Rotation'}</span>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg border text-xs font-semibold shadow-2xs transition-colors ${
                    config.iconFlipH
                ...
                >
                  ...
                  <span>{isAr ? 'عكس أفقي' : 'Flip Horiz'}</span>
                </button>
                ...
                >
                  ...
                  <span>{isAr ? 'عكس رأسي' : 'Flip Vert'}</span>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'text' && (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <span className="w-1 h-4 bg-indigo-500 rounded"></span>
                <span>{isAr ? 'التايبوغرافي وانحناء النصوص' : 'Typography & Curve Controls'}</span>
              </h3>
              <label className="flex items-center gap-1.5 text-xs text-slate-600 font-semibold cursor-pointer">
                <input
                  type="checkbox"
                ...
                />
                <span>{isAr ? 'تفعيل النص' : 'Enable'}</span>
              </label>
            </div>
            ...
            {config.showText && (
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                    {isAr ? 'اسم العلامة / الشعار' : 'Main Text / Brand Name'}
                  </label>
                  <input
                    type="text"
                    value={config.text}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs text-slate-500 font-medium">
                      <span>{isAr ? 'تباعد الحروف' : 'Spacing'}</span>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs text-slate-500 font-medium">
                      <span>{isAr ? 'حالة الأحرف' : 'Case'}</span>
                    </div>
                    <select
                      ...
                      className="w-full bg-white border border-slate-200 rounded-lg px-2 py-1 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none shadow-2xs"
                    >
                      <option value="none">{isAr ? 'عادي' : 'Default'}</option>
                      <option value="uppercase">{isAr ? 'أحرف كبيرة UPPER' : 'UPPERCASE'}</option>
                      <option value="capitalize">{isAr ? 'أول حرف كبير Capitalize' : 'Capitalize'}</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-3 border-t border-slate-100">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                    {isAr ? 'انحناء النص والتأثير المقوس (Text Curve)' : 'Text Curve & Arc'}
                  </span>
                  ...
                  {config.textCurve !== 'straight' && (
                    <div className="space-y-1 pt-1">
                      <div className="flex justify-between text-xs text-slate-500 font-medium">
                        <span>{isAr ? 'نصف قطر الانحناء' : 'Curve Radius'}</span>
                ...
                <div className="space-y-3 pt-3 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-700">{isAr ? 'إطار للنص (Text Stroke)' : 'Text Stroke'}</span>
                    <input
                      type="checkbox"
                  ...
                  {config.textStroke && (
                    <div className="space-y-2">
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs text-slate-500 font-medium">
                          <span>{isAr ? 'سُمك الإطار' : 'Width'}</span>
                      ...
                      <AdvancedColorPicker
                        label={isAr ? 'لون إطار النص' : 'Stroke Color'}
            ...
            <div className="space-y-4 pt-3 border-t border-slate-200">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  {isAr ? 'السلوغان / الشعار الفرعي' : 'Tagline / Slogan'}
                </span>
                <input
                  type="checkbox"
              ...
              {config.showTagline && (
                <div className="space-y-3">
                  <input
                    ...
                    placeholder={isAr ? 'مثال: حلول المستقبل الذكية' : 'e.g. SMART SOLUTIONS'}
                    className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none shadow-2xs"
                  />

                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <span className="text-[11px] text-slate-500 font-medium">{isAr ? 'الحجم' : 'Size'}</span>
                      <input
                        type="range"
                        min="10"
                        max="32"
                    ...
                    <div className="space-y-1">
                      <span className="text-[11px] text-slate-500 font-medium">{isAr ? 'إزاحة عمودية' : 'Offset Y'}</span>
                      <input
                        type="range"
                        min="-50"
                        max="100"
                  ...
                  <AdvancedColorPicker
                    label={isAr ? 'لون السلوغان' : 'Tagline Color'}
        ...
        {activeTab === 'bg' && (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <span className="w-1 h-4 bg-indigo-500 rounded"></span>
                <span>{isAr ? 'شكل القناع والخلفية' : 'Shape Mask & Background'}</span>
              </h3>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                {isAr ? 'شكل القناع (Shape Mask)' : 'Shape Mask'}
              </label>
              <div className="grid grid-cols-3 gap-1.5">
                {[
                  ...
                  <button
                    ...
                    className={`py-2 px-1 text-xs rounded-lg border font-semibold truncate transition-all shadow-2xs ${
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {POPULAR_PALETTES.map((pal) => (
                  <button
                    ...
                    className="flex items-center gap-2 p-1.5 rounded-lg border border-slate-200 bg-white hover:border-slate-300 text-left transition-all shadow-2xs"
                  >
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-lg p-0.5">
                  {[
                    { id: 'transparent', ar: 'شفافة (بدون)', en: 'Transparent' },
                    { id: 'solid', ar: 'لون موحد', en: 'Solid' },
                    { id: 'linear', ar: 'تدرج خطي', en: 'Linear' },
                    { id: 'radial', ar: 'تدرج دائري', en: 'Radial' },
                  ].map((item) => (
                    <button
                      key={item.id}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {(config.bgType === 'linear' || config.bgType === 'radial') && (
                      <AdvancedColorPicker
                        label={isAr ? 'اللون 2 (تدرج)' : 'Color 2 (Gradient)'}
                  ...
                  {config.bgType === 'linear' && (
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs text-slate-500 font-medium">
                        <span>{isAr ? 'زاوية التدرج' : 'Gradient Angle'}</span>
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                {isAr ? 'نقش الخلفية (Pattern)' : 'Pattern Texture'}
              </span>
              <div className="grid grid-cols-5 gap-1">
                {(['none', 'dots', 'grid', 'stripes', 'waves'] as PatternType[]).map((pat) => (
                  <button
                    ...
                    className={`py-1.5 text-xs rounded-lg border font-semibold capitalize transition-all shadow-2xs ${
                      config.pattern === pat
                        ? 'bg-indigo-50 border-indigo-400 text-indigo-700'
                        : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                    }`}
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100">
              <div className="space-y-1">
                <div className="flex justify-between text-xs text-slate-500 font-medium">
                  <span>{isAr ? 'سُمك الإطار' : 'Border Width'}</span>
        ...
        {activeTab === 'layout' && (
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <span className="w-1 h-4 bg-indigo-500 rounded"></span>
                <span>{isAr ? 'نمط التوزيع' : 'Layout & Output Formats'}</span>
              </h3>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                {isAr ? 'نمط التوزيع والمحاذاة' : 'Layout Preset'}
              </label>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`p-2.5 text-xs rounded-xl border font-bold text-left transition-all shadow-2xs ${
            ...
            <div className="mt-4 p-4 bg-slate-900 rounded-xl text-white shadow-md space-y-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-300">
                  {isAr ? 'حزمة Favicon المتكاملة المضمنة' : 'Favicon Bundle Included'}
                </h4>
              </div>
